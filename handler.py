...
def set_global_vars():
    ...
    try:
        global_vars["Owner"]                    = os.environ.get("OWNER")
        global_vars["Environment"]              = "Prod"
        global_vars["aws_region"]               = "us-east-1"
        global_vars["tag_name"]                 = "serverless_cloudwatch_logs_exporter"
        global_vars["retention_days"]           = os.environ.get("RETENTION_DAYS")
        global_vars["cw_logs_to_export"]        = os.environ.get("CLOUDWATCH_LOGS").split(",")
        global_vars["log_dest_bkt"]             = os.environ.get("S3_LOG_BUCKET")
...
def gen_ymd(t,d) -> str:
    ...
    :rtype: json
    """
    ymd =  ( str(t.year) + d + str(t.month).zfill(2) + d + str(t.day).zfill(2) )
    return ymd
